...
def model_init(args):
    ...
    if os.path.isfile(args.checkpoint):
        ...
        if "m0" in checkpoint:
            ...
            m0.load_state_dict(checkpoint["model_state_dict"])
        else:
            try:
                m0.load_state_dict(checkpoint)
            except:
                print(f"Warning: Checkpoint keys: {list(checkpoint.keys())}")
                # Try loading directly if it matches state dict
                pass
